...
func main() {
	...
	mux.HandleFunc("focus:check", func(ctx context.Context, t *asynq.Task) error {
		...
		// Log the focus check for now; in production this would check
		// if channel velocity exceeds threshold and trigger Focus Mode
		slog.Info("focus check completed", "channel", channelID)
		return nil
	})
